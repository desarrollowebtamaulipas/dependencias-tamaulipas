...
jQuery(document).ready(function($) {
	...
	// Funcion para el CTA del Home
	function actualizarFondoCTA() {
	
		// Si no existe #cta-home, no hacer nada
		if ($('#cta-home').length === 0) return;
	
		const isMobile = $(window).width() < 768;
	
		$('#cta-home .bg-splash').each(function() {
			const $el = $(this);
			const imgDesktop = $el.data('img-desktop');
			const imgMobile  = $el.data('img-mobile');
	
			// Seleccionar imagen según tamaño
			const url = (isMobile && imgMobile) ? imgMobile : imgDesktop;
			if (url) {
				$el.css('background-image', `url('${url}')`);
			}
	
			// Agregar o quitar clase según vista
			$el.toggleClass('is-mobile', isMobile);
		});
	}
	
	// Ejecutar al cargar y al redimensionar
	$(window).on('load resize', actualizarFondoCTA);

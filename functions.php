...
// Darle formato de cientos a los datos duros
	...
	add_filter('acf/format_value/name=datos_4_dato', 'fix_number', 20, 3);
	function fix_number($value, $post_id, $field) {
		// Si esta vacío o no es numérico, regresar el valor tal cual
		if (empty($value) || !is_numeric($value)) {
			return $value;
		}
		// Si sí es número, lo formateamos
		return number_format($value);
	}
	
// Formato de campos al 100% en responsivo
	add_action('acf/input/admin_head', function(){ ?>
	<style>
	/* Mobile responsive para repetidores */
	@media (max-width: 782px) {
		...
		th.acf-th[data-name="directorio_foto"]{
			width: 80% !important;
		}
		
		th.acf-th[data-name="directorio_nombre"],
		th.acf-th[data-name="directorio_puesto"]{
			display: none;
		}
	}
	</style>
...
// Actualización por Github
	class DependenciasTamaulipasThemeUpdater {
		private $theme_slug = 'dependencias-tamaulipas-main';
		private $update_url = 'https://raw.githubusercontent.com/desarrollowebtamaulipas/dependencias-tamaulipas/refs/heads/main/update.json';
	...
	new DependenciasTamaulipasThemeUpdater();
